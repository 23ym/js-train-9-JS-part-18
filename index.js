...
function findIndexByProperty(arr, name) {
  if (!Array.isArray(arr)) { 
    return -1; // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо -1
  }
  const foundIndex = arr.findIndex((element) => element.name === name); // Використовуємо метод findIndex для пошуку індексу об'єкта, властивість 'name' якого співпадає з заданим іменем.
...
function findLastEvenNumber(arr) {
  ...
  const lastEvenNumber = arr.findLast((number) => number % 2 === 0); // За допомогою метода findLast знаходимо останнє парне число в масиві.
    return lastEvenNumber !== undefined ? lastEvenNumber : 'Парних чисел не знайдено'; // Повертаємо знайдене число або 'Парних чисел не знайдено', якщо таке число не знайдено.
}
...
function getArrayKeys(arr) {
  ...
  const arrayKeysIterator = arr.keys(); // За допомогою методу keys() отримуємо об'єкт ітератора, який містить ключі масиву.
  const arrayKeys = Array.from(arrayKeysIterator); // Конвертуємо ітератор в масив за допомогою методу from.
    return arrayKeys; // Повертаємо масив ключів.
}
...
function getPositiveArrayValues(arr) {
  ...
  const arrayValuesIterator = arr.values(); // За допомогою методу values() отримуємо об'єкт ітератора, який містить значення масиву.
  const arrayValues = Array.from(arrayValuesIterator); // Конвертуємо ітератор в масив.
  const positiveArrayValues = arrayValues.filter(value => value > 0); // За допомогою методу filter() отримуємо масив лише з додатніми значеннями.
...
function removeAndDouble(arr, startIdx) {
  ...
  const removedElements = arr.splice(startIdx); // За допомогою методу splice() видаляємо елементи масиву починаючи з заданого індексу.
...
function sumAndPrint(arr) {
  ...
  arr.forEach(element => console.log(element)); // За допомогою методу forEach() виводимо кожен елемент масиву в консоль.
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const squaredArray = flattenedArray.map(element => element ** 2); // За допомогою методу map() підносимо кожен елемент масиву до квадрату.
  const filteredSquareArray = squaredArray.filter(element => element > threshold); // За допомогою методу filter() фільтруємо елементи, які більше за порогове значення.
    return filteredSquareArray; // Повертаємо вирівняний масив, елементи якого підняті до квадрату і профільтровані.
}
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = lastNElements.reduceRight((acc, curr) => acc + curr, 0); // За допомогою методу reduceRight() обчислюємо суму останніх n елементів масиву.
  lastNElements.forEach(element => { 
    console.log(element); // За допомогою методу forEach() виводимо кожен елемент з останніх n в консоль.
  });
...
function wordLengths(arr) {
  if (!Array.isArray(arr)) { // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо 'Вхідний параметр має бути масивом'
...
function flattenArray(arr) {
  ...
  const flattenedArray = arr.flat(Infinity); // Використовуємо метод flat з параметром Infinity, щоб "випрямити" масив на будь-яку глибину вкладеності
    return flattenedArray; // Повертаємо "випрямлений" масив
}
...
function expandArrayByN(arr, n) {
  if (!Array.isArray(arr) || typeof n !== 'number') { // Перевіряємо, чи вхідні параметри є масивом та числом відповідно, якщо ні повертаємо 'Перший вхідний параметр має бути масивом, другий - числом'
    return 'Перший вхідний параметр має бути масивом, другий - числом'; 
  }  
  const expandedArray = arr.flatMap((num) => Array(n).fill(num));  // Використовуємо метод flatMap для створення нового масиву, де кожне число повторюється n разів
...
function findLongestWord(arr) {
  if (!Array.isArray(arr)) { 
    return ''; // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо пустий рядок
  }
  const longestWord = arr.reduce((longest, current) => { // Використовуємо метод reduce() для знаходження найдовшого слова
  if (current.length > longest.length) { // Порівнюємо довжину поточного слова з довжиною найдовшого слова
    return current; // Інакше повертаємо попереднє найдовше слово без змін
  } else { 
    return longest;
  }
}, '');
...
function findDuplicateElements(arr) {
  ...
  const duplicateElements = arr.filter((element, index) => { // Використовуємо метод filter() для вибірки лише дубльованих елементів
...
function capitalizeWords(arr) {
  ...
  const capitalizedArray = arr.map((word) => { // Використовуємо метод map() для перетворення кожного слова в рядку з першою великою літерою
    return word.charAt(0).toUpperCase() + word.slice(1); // Перетворюємо першу літеру слова у верхній регістр да додамо до неї всі символи слова крім першого
  }); //Повертаємо слово
...
function calculateTotalPrice(arr) {
  if (!Array.isArray(arr)) { 
    return 0; // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо 0
  }
  const totalPrice = arr.reduce((total, product) => { // Використовуємо метод reduce() для обчислення загальної ціни
    return total + product.price; 
  }, 0); // Додаємо ціну товару до загальної суми та повертаємо результат
